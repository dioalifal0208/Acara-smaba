...
export default function FaceScannerModal({ activeWorkcode, participant, onClose, onSuccess }) {
    ...
    }, [isModelsLoaded, gpsData, errorMsg, result, stopVideoStream]);
    ...
    useEffect(() => {
        if (challenge && !challengePassed && timeLeft > 0 && !result) {
            ...
            setErrorMsg("Waktu habis! Silakan coba lagi.");
            stopVideoStream();
        }
    }, [challenge, timeLeft, challengePassed, result, stopVideoStream]);
            ...
            <div className="w-full max-w-sm w-full rounded-3xl bg-white overflow-hidden shadow-2xl relative" onClick={(e) => e.stopPropagation()}>
                ...
                {result ? (
                    <div className="p-7 text-center">
                        ...
                        <div className="bg-emerald-50 border border-emerald-200 rounded-2xl p-4 mb-4 mt-4">
                        ...
                        <button onClick={handleClose} className="w-full rounded-xl bg-slate-100 py-3 text-sm font-bold text-slate-600 transition-all hover:bg-slate-200">
                            Tutup
                        </button>
                    </div>
                ) : (
