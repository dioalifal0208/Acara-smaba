import{r as I,j as t,H as P,f as M}from"./app-BdEP1xwd.js";import{A as D}from"./AuthenticatedLayout-DJeENLpF.js";import"./transition-BNrW8_UC.js";function R({workcodes:b=[],selectedWorkcodeId:c,selectedWorkcode:s,stats:d,attendances:u=[]}){const[h,y]=I.useState(""),j=e=>{M.get(route("report"),{workcode_id:e},{preserveState:!0,preserveScroll:!0})},m=u.filter(e=>e.nama.toLowerCase().includes(h.toLowerCase())||e.nis_nip.toLowerCase().includes(h.toLowerCase())),g=d.total>0?Math.round(d.hadir/d.total*100):0,v=()=>{if(!s)return;const e=window.open("","_blank","width=800,height=900"),o=m.map((a,p)=>`
            <tr>
        ...
        `).join(""),x=s.kategori==="harian"?`
            ...
            <body>
                ...
                <div class="title-doc">
                    <h2>REKAP PRESENSI ${s.nama_workcode.toUpperCase()}</h2>
                </div>
                ...
                <div class="meta-info">
                    <table>
                        ...
                        <tr>
                            ...
                            <td>:</td>
                            <td>${d.hadir} dari ${d.total} peserta (${g}%)</td>
                        </tr>
                        <tr>
                ...
                <table class="data-table">
                    ...
                    <tbody>
                        ${o||'<tr><td colspan="10" style="text-align:center; padding: 10px;">Belum ada data kehadiran</td></tr>'}
                    </tbody>
                </table>
        ...
        `),e.document.close()},k=async(e,o)=>{if(s)try{const a=await(await fetch(`/report/individual/${s.id}/${e}`)).json(),p=window.open("","_blank","width=800,height=900"),N=a.attendances.map((i,_)=>{let l="-",w="-",f="-";if(i.waktu_hadir&&i.waktu_hadir!=="-"){const n=i.waktu_hadir.split(" ");if(n.length>=4){l=n.slice(0,3).join(" "),w=n[3];const r=new Date(l);isNaN(r)||(l=`${r.toLocaleDateString("id-ID",{weekday:"long"})}, ${l}`)}else l=i.waktu_hadir}if(i.waktu_pulang&&i.waktu_pulang!=="-"){const n=i.waktu_pulang.split(" ");if(n.length>=4){if(f=n[3],l==="-"){l=n.slice(0,3).join(" ");const r=new Date(l);isNaN(r)||(l=`${r.toLocaleDateString("id-ID",{weekday:"long"})}, ${l}`)}}else f=i.waktu_pulang}return`
                    <tr>
                        <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px;">${_+1}</td>
                        <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px;">${l}</td>
                        <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px;">${w}</td>
                        <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px;">${f}</td>
                ...
                `}).join(""),z=a.attendances.filter(i=>i.status==="alpha").length,A=a.attendances.filter(i=>i.status==="izin").length,$=a.attendances.filter(i=>i.status==="sakit").length,L=a.attendances.filter(i=>i.status==="lupa_absen").length,C=a.attendances.filter(i=>i.status==="hadir").length;p.document.write(`
                <!DOCTYPE html>
                <html>
                <head>
                    <title>Rekap Kehadiran - ${o}</title>
                    <style>
                ...
                <body>
                    ...
                    <table class="data-table" style="margin-bottom: 15px;">
                        ...
                        <tbody>
                            <tr>
                                <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px; font-weight: bold; color: #dc2626;">${z}</td>
                                <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px; font-weight: bold; color: #d97706;">${A}</td>
                                <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px; font-weight: bold; color: #2563eb;">${$}</td>
                                <td style="text-align: center; padding: 7px 8px; border: 1px solid #cbd5e1; font-size: 11px; font-weight: bold; color: #475569;">${L}</td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <table class="data-table">
                        ...
                        <tbody>
                            ${N||'<tr><td colspan="5" style="text-align:center; padding: 10px;">Belum ada data kehadiran</td></tr>'}
                        </tbody>
                    </table>
